(ns tatara.methods.social
  "social.cljc — 鑪 (たたら) — 世界製造拠点・物流ジオグラフィ知識グラフ DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from tatara's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "鑪 (たたら) — 世界製造拠点・物流ジオグラフィ知識グラフ が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over tatara's own domain (The geographic / facility-scale layer beneath the kabuto supply-chain KG (org→org edges) and uchiwake product-BOM KG: where the world's…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "tatara R0: live social posting is Council Lv6+ + operator + "
